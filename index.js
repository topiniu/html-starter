...
`;
function drawSVGOnCanvas(svgElement) {
    ...
    img.onload = function() {
        ...
        // Calculate the scaling factor to fit the SVG in the canvas
        const scale = Math.min(viewWidth / img.width, viewHeight / img.height) * 0.8; // 80% of the canvas size
        const x = (viewWidth - img.width * scale) / 2;
        const y = (viewHeight - img.height * scale) / 2;
        ...
        ctx.strokeStyle = '#BD4932';
        ctx.lineWidth = 10;
        ctx.strokeRect(x - 10, y - 10, img.width * scale + 20, img.height * scale + 20);
        ...
        ctx.fillText('Congratulations!', viewWidth / 2, 20);
    };
...
function createModal(messageType) {
    console.log('[createModal]', messageType);
    const parser = new DOMParser();
    const svgDoc = parser.parseFromString(svgContent, 'image/svg+xml');
    const svgElement = svgDoc.documentElement;

    // Draw the SVG on the canvas
    drawSVGOnCanvas(svgElement);
